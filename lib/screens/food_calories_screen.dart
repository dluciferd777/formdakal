// lib/screens/food_calories_screen.dart - RENKLER GÜNCELLENDİ
...
class _FoodCaloriesScreenState extends State<FoodCaloriesScreen> with AutomaticKeepAliveClientMixin {
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                // Özet - RENKLER GÜNCELLENDİ
                Row(
                  children: [
                    _buildStatItem('Kalori', calories.toInt().toString(), 'kcal', Colors.red),     // KIRMIZI
                    _buildStatItem('Protein', protein.toStringAsFixed(1), 'g', Colors.green),     // YEŞİL
                    _buildStatItem('Karb', carbs.toStringAsFixed(1), 'g', Colors.blue),          // MAVİ
                    _buildStatItem('Yağ', fat.toStringAsFixed(1), 'g', Colors.yellow),           // SARI
                  ],
                ),
                
                // Detaylar - RENKLER GÜNCELLENDİ
                if (_isNutritionExpanded) ...[
                  const SizedBox(height: 16),
                  const Divider(),
                  const SizedBox(height: 16),
                  _buildDetailRow('Kalori', Icons.local_fire_department_rounded, calories, 'kcal', Colors.red, isDark),        // KIRMIZI
                  const SizedBox(height: 8),
                  _buildDetailRow('Protein', Icons.fitness_center_rounded, protein, 'g', Colors.green, isDark),               // YEŞİL
                  const SizedBox(height: 8),
                  _buildDetailRow('Karbonhidrat', Icons.grass_rounded, carbs, 'g', Colors.blue, isDark),                     // MAVİ
                  const SizedBox(height: 8),
                  _buildDetailRow('Yağ', Icons.opacity_rounded, fat, 'g', Colors.yellow, isDark),                            // SARI
                  const SizedBox(height: 8),
                  _buildDetailRow('Şeker', Icons.cake_rounded, carbs * 0.3, 'g', Colors.pink, isDark),                      // PEMBE (ayrı)
                ],
              ],
            ),
          ),
  ...
  Widget _buildDetailRow(String name, IconData icon, double value, String unit, Color color, bool isDark) {
    return Row(
      children: [
        Icon(icon, color: color, size: 16), // İKON RENGİ GÜNCELLENDİ
        ...
        Text(
          ...
          style: TextStyle(
            fontSize: 14,
            fontWeight: FontWeight.bold,
            color: color, // YAZI RENGİ GÜNCELLENDİ
          ),
        ),
      ],
    );
